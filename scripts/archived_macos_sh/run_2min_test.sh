#!/bin/bash
# Run 2-min productivity test with live data monitoring
# Game auto-quits at 120s. Data: Tests/playtest_session.jsonl
#
# Usage: ./run_2min_test.sh
...
echo "=== 2-min Productivity Test ==="
echo "Data: $LOG_FILE"
echo "Game will auto-quit at 120s. Play normally (place claim, gather, herd)."
echo ""
...
# Tail log in background; when Godot exits (120s), we stop
(
  tail -f "$LOG_FILE" 2>/dev/null | while read -r line; do
    if echo "$line" | grep -q '"evt":"snapshot"'; then
      ...
      echo "--- Test ended (120s) ---"
    fi
  done
) &
TAIL_PID=$!

# Wait for Godot to exit (auto-quit at 120s)
