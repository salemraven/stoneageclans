...
- Godot's Project Manager stores RECENT paths in your user profile; the old absolute path is a dead link.
- Preferred fix: Project Manager -> Remove Missing -> Import -> pick the folder that contains project.godot (StoneAgeClans).
- Or double-click Open_In_Godot_Editor.cmd in the project root (passes --path explicitly).
- Cursor: File -> Open Folder -> StoneAgeClans.
- Avoid creating a directory junction as a workaround; it duplicates path names and confuses tooling. Use Import instead.
