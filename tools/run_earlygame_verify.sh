...

{
	...
	echo ""
	echo ">>> [1/6] instrumented playtest (Main smoke, ~4s)"
	bash "$ROOT/tools/run_instrumented_playtest.sh"

	echo ""
	echo ">>> [2/6] ChunkUtils invariants (headless script)"
	"$GODOT" --path "$ROOT" --headless --script res://tools/chunk_utils_verify.gd 2>&1

	echo ""
	echo ">>> [3/6] territory brain integration + JSONL field checks"
	bash "$ROOT/tools/run_territory_brain_integration_verify.sh"

	echo ""
	echo ">>> [4/6] Nomad Mode headless tests (campfire wood burn, panic, march rules)"
	bash "$ROOT/tools/run_nomad_mode_test.sh"

	if [[ "${SKIP_REPRO_HARNESS:-}" == "1" ]]; then
		echo ""
		echo ">>> [5/6] reproduction harness (Player + 2 births) — SKIPPED (SKIP_REPRO_HARNESS=1)"
	else
		echo ""
		echo ">>> [5/6] reproduction harness (headless --repro-harness, ~12–15s)"
		bash "$ROOT/tools/run_repro_harness.sh"
	fi

	if [[ "${SKIP_CLAN_BRAIN_TEST:-}" == "1" ]]; then
		echo ""
		echo ">>> [6/6] ClanBrain JSONL validation — SKIPPED (SKIP_CLAN_BRAIN_TEST=1)"
	else
		echo ""
		echo ">>> [6/6] ClanBrain JSONL validation (~15s Main + capture)"
