...
## 4. What We Built (Artifacts)

| File | Purpose |
|------|---------|
| `scenes/Test3D.tscn` | Root: Control + ColorRect + SubViewportContainer |
| `scenes/test3d_world.tscn` | 3D content: Ground, MarkerCube, TestPlayer, Camera, Light |
| `scripts/test_player_3d.gd` | WASD movement for CharacterBody3D |
| `tools/run_test3d_vulkan.ps1` | Run Test3D with Forward+ |
| `tools/run_test3d_angle.ps1` | Run Test3D with ANGLE driver |
| `tools/run_renderer_diagnostic.ps1` | Interactive renderer comparison |
| `tools/monitor_test3d_realtime.ps1` | Tail realtime log |
| `test3d_run.log` | Old run output (YBot=false, Placeholder=true) |

**project.godot changes:**
- `renderer/rendering_method` toggled between `forward_plus` and `gl_compatibility`
...
- [ ] Keep 2D sprite pipeline as primary
...
## 7. If 2D Shows Black/Grey Screen

If the main 2D game shows a black or grey screen (e.g. on RTX 50 series), try `gl_compatibility` renderer. Copy from `project.godot.renderer_backup` or add under `[rendering]`:
```
renderer/rendering_method="gl_compatibility"
renderer/rendering_method.mobile="gl_compatibility"
```

## 8. If We Return to 3D
...
- [rig.md](Phase4/rig.md) — Transform-based modular rig (2D, not 3D)
