...
- After first green run of the raid test, run the reporter and record: combat_started range, hit/whiff range, target_switch, FPS (e.g. "combat_started: 40–60, hit: 80–150, whiff: 10–30, FPS ≥ 30"). **Green** in later steps = within ~20% of baseline or better. See **bible/AgroCombatBaseline.md** for a template and how to run the reporter.
...
- **Test:** Run `--agro-combat-test`; run reporter; confirm events and counts. Record **baseline** in bible/AgroCombatBaseline.md. Re-test until green. **Sign off; tag/branch.**
