...
**RTS / squad movement** (ordered clansmen, stances, formations) is documented in **[`bible/rts.md`](rts.md)**. This file covers **locomotion** and how formations plug into steering.
...
Full behavior, controls, and stance strategy: **[`bible/rts.md`](rts.md)**.
...
**Deer (`flee_prey`):** **Burst panic sprint** applies **`NPCConfig.deer_flee_burst_speed_mult`** during the **burst** phase; **winded** applies **`deer_winded_speed_mult`**; leaving **`flee_prey`** restores base speed. Details: **`bible/Phase4/raiding_hunting.md`** §5, **`bible/wildlife_movement.md`**.
...
2. Crafting/gathering/hut build timer → `velocity = 0` (must stay in place until timer finishes; only agro/combat/defend/flee may interrupt)
...
Combat/agro skips the npc_base distance throttle so fights stay responsive. Tune both in `NPCConfig`; see also [`bible/phase2/STATE_PRIORITIES.md`](phase2/STATE_PRIORITIES.md).
