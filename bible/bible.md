...
**Canonical definitions:** `bible/game_dictionary.md` (updated for player-facing and design vocabulary). The table below is a quick index + implementation aliases; if anything conflicts, the dictionary wins.
...
- **Code constraint:** Player → clansmen RTS stays **minimal and modular** — shared `command_context`, single formation slot pass, config-driven tuning (`RTS_CONFIG`, `STANCE_CONFIG`) — so behavior stays **maintainable and optimizable**. See **`bible/rts.md`** for the full RTS doc.
...
### Chunk grid (`ChunkUtils` autoload)

Verified constants in `scripts/world/chunk_utils.gd`:

| Constant | Value | Role |
|----------|-------|------|
| `TILE_SIZE` | 64 px | Tile alignment |
| `CHUNK_TILES` | 32 | Tiles per chunk edge |
| `CHUNK_SIZE` | 2048 px | `TILE_SIZE × CHUNK_TILES` |
| `ROAM_RADIUS` | 1638.4 px | Wildlife roam cap (`CHUNK_SIZE × 0.8`) |
| `HOME_UPDATE_TIME` | 30 s | Wildlife home-target refresh |
| `CLAN_AVOID_RADIUS` | 600 px | Spawn/roam avoid other claims |
| `WOMAN_CLAN_AVOID_RADIUS` | 800 px | Women spawn avoid claims |

**Seeded world:** layout and procedural rolls should use **`world_seed + chunk_coords`** (and layer id), not bare `randf()` — required for multiplayer-safe static content. **Player mutations** (picked, chopped, built) are authoritative server/save deltas (`MutationStore`); see **`bible/game_map.md`** and §XX-A.

**Streaming:** chunk load/unload follows local interest today; MP needs union-of-peers interest (not shipped). New static props should be **chunk-scoped** + seeded per project rules.
...
- **The herder who delivers a woman into the claim builds her Living Hut** (timed build, no resource cost in current sim).
- **That same herder is the designated father** for her children until he dies or leaves the claim; then mate search picks a new eligible male in the claim.
...
Detail: **`bible/wildlife_movement.md`**.
...
**Two paths:** NPC clans are brain-driven; the **player** uses RTS hunt modes. Sheep, goats, and women are **never** AoH hunt targets — they use **herd / search** (§XV).

### Area of Hunt (AoH)

- **Land claims only:** `AreaOfHunt` (`land_claim.gd`) — circle **wider than** claim radius; prey inside the **inner claim disk** are excluded (breaks defend/flee/hunt).
- **Counts:** `get_huntables_in_aoh()` — wild **`NPCConfig.is_ai_hunt_prey_type`** (deer, mammoth).
- **Campfires** have no AoH ring.

### NPC clan hunts (ClanBrain → `hunt_state`)

1. Brain evaluates food pressure (`food_days_buffer`, meat/hide counts), AoH prey, fighter count.
2. **`_start_hunt()`** sets **`hunt_intent`** (states: `NONE` → `RECRUITING` → `ACTIVE` → `LOOTING` → `RETREATING`).
3. Fighters pull intent and enter **`hunt_state`** phases: **FORMING → CHASING → KILLING → LOOTING → RETURNING** (mirror of raid party assembly).
4. **Survival mode:** fewer than **2 fighters** → no hunt/raid; gather/herd focus.
5. **Player-owned claims:** brain skips hunt orchestration (defense/threat parity may still run).

Edge cases: **`docs/hunting_system_edge_cases.md`**. Phase design: **`bible/Phase4/raiding_hunting.md`**, **`bible/ai_clan_brain.md`**.

### Player-led hunting (RTS)

- HUD modes: **PEACE · AGRO · HUNT** (one active). Stance row changes per mode (e.g. HUNT: **AMBUSH · STALK · ARC**).
- **H** in **HUNT** mode **aborts** the hunt (switches to PEACE, loud cue — spooks prey); **H** in PEACE/AGRO still **rallies** fighters.
- Deer use **`flee_prey_state`** + perception centroid / sound — see §XVIII and **`bible/Phase4/raiding_hunting.md`**.

### Known tuning gaps (May 2026)

Headless **ClanBrain reports** often show strong population growth but **low food_days_buffer**, **one hunt per clan per run**, and **parties not disbanding** (formed − disbanded > 0). Treat as active balance/FSM work, not doc-only.

---

## XVI. ClanBrain (AI Strategy)

One **ClanBrain** per **territory** node (`land_claim.gd` or `campfire.gd` — same script; campfire uses **nomadic** tuning). RefCounted; sets intent; NPCs **pull** quotas and self-assign. Designed to scale for large villages (many huts, large radius).
...
### Hunt intent states (brain-side)
`NONE` | `RECRUITING` | `ACTIVE` | `LOOTING` | `RETREATING` — drives recruitment and cleanup; NPC **`hunt_state`** runs tactical phases (§XV-A).
...
- **Survival mode:** AI fighters **&lt; 2** → cancel hunt/raid; gather/herd only (`SURVIVAL_MODE_THRESHOLD`).
- **Player clans**: defender ratio from slider; **no raid/hunt evaluation** (player RTS + defend slider).
...
Authoritative detail: **`bible/phase2/STATE_PRIORITIES.md`**.
...
6. Defend (8.0)
7. Raid (8.5)
8. **Hunt** (`hunt_state` — AI clan hunt parties) (8.5 band; see state file)
9. Reproduction (7.5)
10. Work at building (7–10)
11. Gather (5.6–6.0)
12. Herd Wild NPC searching (5.5)
13. **Flee prey** (`flee_prey` — deer/mammoth panic from human threats)
14. Wander (0.5–3.0); **returning_from_break** (after RTS **Break**) can evaluate **~13** so clansmen walk home before herd/gather steal the FSM.

---

## XVIII. RTS Controls (Player → Clansmen)

**Design premise:** Orders are **primitive** (gesture, horn, stance), not a modern command vocabulary — see **§I Primitive command model**. **Full detail:** `bible/rts.md` (controls, files, playtest flags, engineering rationale).
...
|------|-----------------|-----|
| **PEACE** | FOLLOW, GUARD, HIDE | Travel, escort, crouch in cover |
| **AGRO** | ATTACK, GUARD, AMBUSH | Fight push; ambush releases on leader melee windup |
| **HUNT** | AMBUSH, STALK, ARC | Stalk/arc on prey; quiet wide rear arc; half-moon ahead |

### Stances (PEACE / AGRO — core three)
...
**HUNT stances** (STALK ~0.5× travel, ARC curved ahead, AMBUSH/HIDE use cover + `hide_state`) — see **`bible/rts.md`** §11a and Phase4 guide.
...
- **Guard** (**0.75×**) is optional for a **tenser** approach; still slower than **Follow** for pure travel. Detail: **`bible/rts.md` §4.4**.
...
├── campfire.gd             # Nomadic base (250px, 6 slots, ClanBrain nomadic mode)
├── gatherable_resource.gd  # Resource nodes, reserve/release, cooldown, ResourceIndex registration
├── network/
│   ├── network_manager.gd  # WebSocket server/client (MP foundation)
│   └── game_sync.gd        # Spawn zones, world_seed, MutationStore snapshot
├── ai/
│   ├── clan_brain.gd       # RefCounted AI strategy (defender/searcher/raid/hunt intent, pressures)
...
│   ├── fsm.gd              # Priority-based state machine; eval every 0.1s
...
│   │   ├── hunt_state.gd   # AI clan hunt party (FORMING→RETURNING)
│   │   ├── flee_prey_state.gd # Deer/mammoth flee from human threats
...
| PlaytestInstrumentor | `logging/playtest_instrumentor.gd` | Event capture; `snapshot` has `state_counts`, `ai_caveman_states` / `ai_clansman_states` (AI only, excludes player), `ai_clans` (per AI claim: leader `state`, `herded_count`, `clan_pop`, `brain`). Reports: `scripts/logging/clanbrain_report.py` (§XX-B) |
...
| ChunkUtils | `world/chunk_utils.gd` | World chunk coords, roam/avoid radii (§II) |
...
1. Territory (`land_claim` or `campfire`) owns `clan_brain`; calls `clan_brain.update(delta)` every frame
2. Every 5s: `_evaluate_clan_state()`, `_update_pressures()`, `_update_defender_assignments()`, `_update_searcher_assignments()`; AI clans also `_update_hunt()` / raid eval (skipped for player-owned and survival mode)
3. Writes `defender_quota`, `searcher_quota`, `raid_intent`, `hunt_intent`, pressures, clan metrics to territory meta
...
7. `hunt_state.can_enter()`: hunt intent active; `npc_join_hunt` / `npc_leave_hunt` on brain

**AoH (land claim):** `AreaOfHunt` body_entered/exited maintains huntable prey list; `get_huntables_in_aoh()` feeds ClanBrain hunt start.
...
## XX-A. Multiplayer & authority (target)

Stone Age Clans is pivoting to **multiplayer-first** simulation. Today most logic still runs in single-player Main; networking is **partial**.

### Non-negotiable principles

| Principle | Meaning |
|-----------|---------|
| **Server authoritative** | Combat, inventory, herd attach, hunt/raid outcomes resolve on server — not client trust. |
| **Fixed simulation tick** | Gameplay timers and AI commits use tick time, not frame delta. |
| **Commands in, events out** | Clients send intents (`CommandMove`, transfer requests, RTS orders); server emits deltas/events. |
| **Interest management** | Replicate entities in spatial/chunk radius per peer (see §II). |
| **Deterministic static world** | Chunk props from **seed + coords**; mutations are server-owned diffs. |

### What exists in repo (snapshot)

| Piece | Status |
|-------|--------|
| `scripts/network/network_manager.gd` | WebSocket peer create server/client |
| `scripts/network/game_sync.gd` | Spawn zones, `world_seed`, `MutationStore` snapshot on join |
| Player movement RPCs | Stub / partial |
| Chunk streaming MP | Local player interest only |

Roadmap detail: **`bible/multiplayer.md`**, planning doc `MULTIPLAYER_AND_ENGINEERING_PLAN.md` (soc planning workspace). Herding refactor target: single **`HerdAuthority`** server subsystem (engineering plan §5).

---

## XX-B. Verification & playtests

**Standard AI tuning loop:**

1. Run Main with `--playtest-capture` (e.g. `bash tools/run_clanbrain_report_5min.sh`).
2. JSONL → `python3 scripts/logging/clanbrain_report.py` → `Tests/logs/clanbrain_report_<ts>/clanbrain_report.md`.
3. Read **Summary**, **Gates** (invariant failures, stuck parties = formed − disbanded), per-clan hunt/food/pressure.

Specs: **`bible/clanbrain_report.md`**, **`bible/Ultimate_npc_clanbrain_test.md`**, **`bible/PLAYTEST.md`**.

Use a fixed **`--playtest-world-seed`** when comparing patches.
...
|----------|--------|
| Core gameplay | Solid |
| AI / ClanBrain | Strong; hunt pipeline **in progress** (food buffer, party disband) |
| NPC hunt parties (AI clans) | Partial — AoH + hunt_intent + hunt_state wired; tuning ongoing |
| Player RTS hunt modes | Partial — PEACE/AGRO/HUNT + stances; spear ambush throw may be incomplete |
| Combat | Working |
...
**Dev resources:** `bible/README.md` (all guides), `bible/dev_resources.md` (playtest + Cursor plans).
...
| **roadmap.md** | Phase 1–3 roadmap to beta. | Planning only; use with issue tracker / `bible/game_map.md` for ordering feature work. |
...
*Sources: gdd.md, main.md, hominids.md, traits.md, Buildings.md, items_guide.md, game_dictionary.md, HERDING_SYSTEM_GUIDE.md, ai_clan_brain.md, clanbrain_report.md, Phase4/raiding_hunting.md, phase1.md, phase2.md, earlygame.md, wildlife_movement.md, game_map.md, **bible/rts.md**, rtsguide.md, AgroGuide.md, GatherGuide.md, DragAndDropInventoryGuide.md, movement.md, SOSA.md, Art_Direction.md, village.md + scripts/ (main.gd, clan_brain.gd, hunt_state.gd, flee_prey_state.gd, party_state.gd, herd_state.gd, formation_utils.gd, party_command_utils.gd, rts_formation_config.gd, fsm.gd, task_runner.gd, combat_scheduler.gd, combat_tick.gd, combat_component.gd, gather_job.gd, resource_index.gd, land_claim.gd, campfire.gd, chunk_utils.gd, clanbrain_report.py, npc_config.gd, playtest_instrumentor.gd, project.godot)*
