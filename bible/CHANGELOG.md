...
- All guides moved into **`bible/`** next to **`bible/bible.md`**.
- Repo root **`bible.md`** and **`guides/README.md`** are redirect stubs only.

### Bible + guides hygiene pass

- **`bible.md`** — May 2026 consolidation: hunting (§XV-A), chunk constants, campfire ClanBrain, MP/ playtest appendices, scope rule (future-only in §XXII).
- **`bible/README.md`** — New index (canonical vs future, links by system).
- **`bible/hunting.md`** — New hub for NPC AoH hunts + player RTS hunt modes.
- **`bible/main.md`** — War Horn + RTS hunt modes marked implemented; ClanBrain on campfire; references refreshed.
- **`bible/ai_clan_brain.md`**, **`gdd.md`**, **`dev_resources.md`**, **`raid.md`**, **`HERDING_SYSTEM_GUIDE.md`**, **`rts.md`** — Aligned with bible; dates bumped.
- **`bible/future implementations/main.md`**, **`warhorn.md`** — Banners: aspirational vs shipped rally.
