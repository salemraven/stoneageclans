...
The herding system allows cavemen (and the player) to bring wild NPCs (women, sheep, goats) into their territory (land claim or campfire radius), where they join the clan. The system is **animal-authoritative**: each herdable has a HerdInfluenceArea (Area2D) that detects nearby herders; animals attach when herders enter range and influence exceeds a threshold. Herders do not own targets—they search or lead. This design enables competition between multiple cavemen and the player.
...
- `bible/Phase4/herd4.md` – Canonical definitions (herd4)
