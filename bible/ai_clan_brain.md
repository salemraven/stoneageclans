...
- Sets **defender** and **searcher** quotas on the **territory** (land claim or campfire; NPCs self-assign via pull-based model)
...
- **Territory** calls `clan_brain.update(delta)` every frame.
- Each frame: **alert decay**; for NPC clans (not player-owned, not survival mode): **`_update_raid()`** and **`_update_hunt()`**.
- **Survival mode:** fighters (cavemen + clansmen) **&lt; `SURVIVAL_MODE_THRESHOLD` (2)** → cancel hunt/raid; gather/herd focus only.
...
Player-led warbands already use RTS **Peace / Agro / Hunt** stances separately (see **`bible/Phase4/raiding_hunting.md`**). This section is **AI clans**:
...
- **Player hunt HUD & deer behavior:** `bible/Phase4/raiding_hunting.md`
- **Combat / agro:** `bible/AgroGuide.md`, `scripts/npc/states/combat_state.gd`
