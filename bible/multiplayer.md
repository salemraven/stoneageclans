...
| Area | Status |
|------|--------|
| **`scripts/network/network_manager.gd`** | **Done** — `WebSocketMultiplayerPeer` create_server / create_client; signals `peer_connected_to_game` / `peer_disconnected_to_game`. |
| **`scripts/network/game_sync.gd`** | **Partial** — Spawn zones from `WorldGenConfig.player_spawn_zones` (chunk centers via `ChunkUtils`); `consume_spawn_world_position_for_peer`; `apply_spawn_position` RPC; `receive_world_snapshot` (applies `world_seed` + `MutationStore` dict). **Main** connects server hooks to send spawn + snapshot on join. |
| **Player movement / state RPCs** | **Stub** — `broadcast_player_state` etc. still placeholders; not full 20 Hz sync. |
| **Chunk streaming + MP** | **Single-player interest only** — `ChunkManager` follows the **local** player’s chunk disk; **no union-of-all-peers** interest yet. See **`bible/game_map.md` §14**. |
| **World spawn sync (Phase 5 below)** | **Partial** — Minigame NPCs + chunk content run on server today in SP; full **network_id** spawn messages and client-only replicas are **not** done. |

Checklist sections below remain the **target roadmap**; treat `[ ]` as “not complete for shipped MP” unless you mark them yourself after testing.
...
- [x] **`scripts/network/network_manager.gd`** — WebSocket peer + `start_server` / `connect_to_server` (see file for API).
- [x] Use `WebSocketMultiplayerPeer` (browser path)
- [x] Server: listen on port (default **9080** in script)
- [x] Client: connect to `ws://host:port`
- [x] Integrate with Godot `MultiplayerAPI` (`multiplayer.multiplayer_peer` assigned in NetworkManager)
...
- See `bible/future implementations/perception_rpc.md` for when to add RPC sync.
...
| Action | Path |
|--------|------|
| Done | `scripts/network/network_manager.gd` |
| Partial | `scripts/network/game_sync.gd` — spawn zones + snapshot; extend for entity/state sync |
